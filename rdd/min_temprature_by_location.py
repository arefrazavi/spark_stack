...
if __name__ == '__main__':
    spark_conf = SparkConf().setMaster('local[*]').setAppName('MinTemperatureByLocation')
    ...
    location_min_type_temperature_rdd = location_type_temperature_rdd.filter(lambda item: 'MIN' in item[1])
    ...
    min_temperature_by_location = location_temperature_rdd.reduceByKey(lambda temp_1, temp_2: min(temp_1, temp_2))

    for element in min_temperature_by_location.collect():
        print(element[0], ': ', str(element[1]) + 'C')
