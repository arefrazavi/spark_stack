...
def extract_datasets(spark: SparkSession) -> (DataFrame, DataFrame):
    ...
    # Change path to s3n://sundog-spark/spark_ml-1m/ratings.dat and sep='::'
...
def cal_movies_similarities(movie_ratings_df: DataFrame):
    ...
    and the similarity of their ratings when watched by same person.
...
if __name__ == '__main__':
    ...
    # Change: Remove master('local[*]') to take advantage of built-in configuration on EMR elastic map reduce
    # that will automatically tell spark to run on top of Hadoop yarn using the cluster that I've created using EMR.
